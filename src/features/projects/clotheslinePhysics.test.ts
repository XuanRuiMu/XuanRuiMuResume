...
describe('晾衣架物理引擎 - 质量差异', () => {
  ...
  it('便签密度高于绳节点密度且便签质量远大于绳节点', () => {
    const 检视 = 引擎.获取刚体检视()
    const 绳节点 = 检视.主绳节点[5]
    const 便签 = 检视.便签[0]
    expect(便签.density).toBeGreaterThan(绳节点.density)
    expect(便签.mass).toBeGreaterThan(绳节点.mass * 10)
  })

  it('绳节点阻尼大于便签（轻绳风停即稳、重签摆动悠长）', () => {
    const 检视 = 引擎.获取刚体检视()
    const 绳节点 = 检视.主绳节点[5]
    const 便签 = 检视.便签[0]
  ...
  it('风停后重便签因惯性保持摆动（低阻尼衰减慢）', () => {
    ...
    // 15 帧（约 0.25s）后仍保留大部分摆速，体现惯性；高阻尼刚体此时应已近静止
    expect(衰减签速).toBeGreaterThan(停风签速 * 0.3)
  })
})
